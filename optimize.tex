...
elimination. Moreover, the
second input group is now cyclic and we can hash into it.
The only potential drawback
is that there is no known efficient method
for mapping elements of one input group to the other,
which could complicate security proofs or even render the pairing
unsuitable for certain cryptosystems.
...
We wish to avoid computing in $\Fqk$
...
a good choice is an $r$ that has
the form $2^a \pm 2^b \pm 1$ (Solinas numbers) or has
low Hamming weight~\cite{bakls}.
