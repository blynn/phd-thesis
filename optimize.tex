...
Recall for Type A, B and C curves we can use distortion maps to construct
symmetric pairings whose input groups are cyclic and lie over smaller
fields.
...
In practical cryptosystems, picking the second input randomly from
$E(\Fqk)$ or $E(\Fqk)[r]$ (depending on whether the Tate or Weil pairing is
used) will most likely result in a nontrivial pairing output and for the
most part it does not
matter that $E(\Fqk)$ is not cyclic, though security proofs do need
to be rewritten.
...
arithmetic involved for these operations can be performed in $\Fq$ rather
than $\Fqk$.

In the literature, this is known as a Miller-lite operation.
...
Eventually, we must evaluate equations of lines at points of $E(\Fqk)$.
If distortion maps or twist maps are being used, they must be evaluated
on the second input point $Q$ to obtain a point on $E(\Fqk)$. However
this should be done carefully:

\subsection{Last-Second Conversions}
...
savings in some cases. In general, we wish to avoid computing in $\Fqk$
...
Since group inversion (point negation) is practically free,
...
Denominator elimination allows us to ignore vertical lines, and as
stated above the $G_T$
inversions can be collated into one (by keeping separate
numerator and denominator variables during the main loop and only
dividing at the end).

Thus addition-subtraction-chain exponentiation can be
better than simply addition-chain exponentiation and as with
