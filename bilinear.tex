...
commonly used by cryptographers.
...
and hashing to a group
element must all be efficient.
(Choosing a random element should be efficient as well, but this is
implied by fast multiplication: one can simply choose
a random integer and exponentiate a generator by this power.)
...
curves~\cite{galbraith,silverberg} can be used but there is no compelling
reason to do so.
