...
Let $P=(x, y) \in E(K)[r]$, $Q=(x_2, y_2) \in E(K)$ ($Q$ represents a
coset).
Define constants
\[
    C = (2 y)^{-1}
\]
\[
    A = (x - x^2)^{-1}, B = {({(2x + x_2)(x - x_2)^2 - (y + y_2)^2})}^{-1},
\]
...
\end{array}
\]
Then it can be shown that the Tate pairing
\[
e : E[r] \cap E(K) \times
E(K) / r E(K) \rightarrow
K^* / K^{*r}
...
e(P,Q) = d_{r+1}/c_{r+1}.
\]
We describe a repeated-squaring-style algorithm that uses these sequences
...
This example is artificially small, and we do not in fact need to bother
computing $c_7, c_8, c_9$, and many sequence values
...
should be exploited in an implementation. For example,
setting
\[
