...
Divisors are merely a notational tool to express zeroes and poles and their
multiplicities, or \emph{orders}.
Rather than waste space writing about zeroes and poles $P_1,...,P_n$
of orders $a_1,...,a_n$, where $P_i$ is a zero of multiplicity
$a_i$ if $a_i > 0$ and is a pole of multiplicity $-a_i$ otherwise,
one can simply write down the following \emph{divisor}:
\[ D = a_1\langle P_1\rangle + ... + a_n \langle P_n\rangle .\]

We break with tradition and instead use multiplicative notation for the divisor
group. Additionally, we omit the number of poles or zeroes at the point $O$.
For divisors of rational functions, we do not lose any information because its
order is the negation of the sum of the orders of the other points.

Let $D$ be the divisor with zeroes and poles $P_1,...,P_n$
of multiplicities $a_1,...,a_n$. Instead of the above, we write $D$ as
\[ D = (P_1)^{a_1} ... (P_n)^{a_n} . \]

Our nonstandard notation has several advantages. Using additive group notation
for the elliptic curve and multiplicative notation for divisors reduces the
likelihood of confusing the two groups.
Also, leaving out the $(O)$ term allows us to focus less on bookkeeping and
more on the task at hand: in any case, the order of $(O)$ could be readily
calculated if we cared. Lastly, consider taking the product of two functions.
Let $(f)$ denote the divisor of $f$. Then for any rational functions $f, g$ we
have $(f)(g) = (f g)$.
...
of $f_P$ every line equation has coefficients lying in $\Fq$, whereas
we had to perform arithmetic in the larger field $\Fq[i]$
to compute the equations of the lines, tangents and verticals of $f_Q$.
