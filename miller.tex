...
We omit the details since we do not wish to discuss
projective geometry, but it turns out that every line
...
For divisors of rational functions, we actually do not lose any information 
because if we cared,
we could always recover the order of $(O)$
simply by negating of the sum of the orders of the finite points.
...
notation for the elliptic curve and multiplicative notation for divisors
emphasizes the difference between them and reduces the likelihood of
...
Intuitively, Weil reciprocity can be thought of as a generalization
of the much simpler version of this statement for polynomials: given
two polynomials $p, q$, the result
of evaluating $p$ at the zeroes of $q$ and multiplying
...
for any $R$ we have $f_P(Q) = f_P(Q+R)/f_P(R)$
