...
\begin{algorithm}
\caption{(Miller) Tate pairing}
\begin{algorithmic}[1]
\STATE $x \gets 1$
\STATE $Z \gets P$
\FOR {$i \gets t-1, ..., 0$}
    \STATE $x \leftarrow x^2 \cdot T_Z(Q) / V_{2Z}(Q)$
    \STATE $Z \leftarrow 2Z$
    \IF {$r_i = 1$}
	\STATE $x \leftarrow x \cdot L_{Z,P}(Q) / V_{Z+P}(Q)$
	\STATE $Z \leftarrow Z + P$
    \ENDIF
\ENDFOR
\end{algorithmic}
\end{algorithm}
...
\[ e(U, V) = f(U, \phi(V)) \].
Note that $U, \phi(V)$ are always linearly independent thus
we have a nondegenerate bilinear map with cyclic input groups.
(If we worked with the Weil pairing directly then both input groups would be
$E[r]$, which is not cyclic.)
