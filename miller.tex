...
This has 60 points, and we work inside a the subgroup $G$ of size $r = 5$
which can be generated by $P = (25, 30)$:
\[ G =
E(\F_{59})[5] =
...
In practice we wrap the Weil pairing in another map $e$:
\[ e(U, V) = f(U, \phi(V)) \].
This is a nondegenerate bilinear map on $G \times G$.
If we worked with the Weil pairing directly then each input group would be
...
It turns out for our choice of curve, it suffices to compute
...
\[ V_2 : X - 35 \]
As stated before
there are other choices for the equations.
...
but it is a coset representative, and we can standardize the representative
by raising it to the power of $(q^k - 1)/r$:
\[ f_P(Q)^{(q^k-1)/r} = (43+52i)^{(59^1 - 1)/5} = 42+40i \]
\end{enumerate}

As expected, $(42 + 40i)^5 = 1$.

It is not difficult to see that these
degenerate cases always appear in the last iteration
of the main loop whenever $r$ is odd.
