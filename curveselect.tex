...
Happily, it turns out that among the pairings we cover are
symmetric pairings with cyclic input groups that can be
hashed to, and pairings where $r$ can be specified, both of which can be
useful features.

Recall the Tate pairing must be computed in some field extension
$L$ of $K$ that contains the $r$th roots of unity (so $r$ divides $|L| - 1$).
On the other hand,
the Weil pairing is defined on $E[r]$, the $r$-torsion points of $E$,
