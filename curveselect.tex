...
while the security of the resulting systems depends on the size of $r$.
...
Let $k$ be the smallest positive integer such that $\#E(\Fq) | q^k - 1$.
...
divide $q^j - 1$, we have $r | q^j - 1$, that is,
...
$k$ one should manually check if $r | q^j - 1$ for some $j < k$.
